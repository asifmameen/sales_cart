...
describe("EditDiscountDialog", () => {
    ...
    const setup = (initial = baseInitial) => {
        ...
        render(_jsx(EditDiscountDialog, { open: true, initial: initial, onSave: onSave, onClose: onClose }));
    ...
    it("prepopulates fields and allows saving edits", async () => {
        ...
        const amountOpt = within(listbox).queryByText(/€\s*euro/i) ??
    ...
    it("keeps monthly scope and updates duration/description", async () => {
        ...
        if (desc) {
            ...
            await userEvent.type(desc, "first 6 months promo");
        }
        await userEvent.click(screen.getByRole("button", { name: /save/i }));
        const payload = onSave.mock.calls[0][0];
